...
# 读取数据
land_data = pd.read_excel('./C/附件1.xlsx')  # 地块信息
crop_data = pd.read_excel('./C/附件2.xlsx',sheet_name='2023年统计的相关数据')  # 作物产量/成本/价格
demand_data = pd.read_excel('./C/附件1.xlsx')  # 需求量

# 去除空格并确保唯一
land_types = land_data['地块名称'].str.strip().unique().tolist()
crop_data['组合编号'] = crop_data['作物编号'].astype(str) + '_' + crop_data['地块类型']  # 修正换行错误
...
def initialize_population():
    ...
    for _ in range(POPULATION_SIZE):
        ...
        for i, land in enumerate(land_types):
            land_area = land_data.loc[land_data['地块名称'] == land, '地块面积'].values[0]  # 修正换行错误
        ...
        for k, crop in enumerate(crop_types):
            for j in seasons:
                for t in years:
                    ...
                    # 约束条件 3：作物种植与二元变量关联
                    if area > land_data.loc[land_data['地块名称'] == land, "地块面积"].values[0] * z_val:  # 修正"land"拼写错误
...
def differential_mutation(population):
    ...
    for i, target in enumerate(population):
        ...
        donor = {'x': np.copy(population[r1]['x']), 'z': np.copy(population[r1]['z'])}  # 修正换行错误
        donor['x'] = population[r1]['x'] + MUTATION_FACTOR * (population[r2]['x'] - population[r3]['x'])  # 修正换行错误
        # 确保生成的解在合理范围内
        for i in range(len(land_types)):
            land_area = land_data.loc[land_data['地块名称'] == land_types[i], '地块面积'].values[0]  # 修正"地块面积"空格错误
            donor['x'][i] = np.clip(donor['x'][i], 0.5 * land_area, 1.0 * land_area)  # 修正"land_area"拼写错误
...
print(output_data)  # 保存结果到Excel可补充pd.to_excel逻辑
